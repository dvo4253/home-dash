...
export const changeTargetTemp = (action$, store) => action$
	.pipe(ofType(INCREMENT_TARGET_TEMP, DECREMENT_TARGET_TEMP),
		map((action) => {
		...
		mergeMap(targetTemp => [{
			type: UPDATE_UI_TARGET_TEMP,
			payload: { targetTemp },
		},
		{ type: UPDATE_NEST_TARGET_TEMP, payload: { targetTemp } }]));

export const updateNestTargetTemp = (action$, store) => action$
	.pipe(ofType(UPDATE_NEST_TARGET_TEMP), debounceTime(1000),
		map((action) => {
...
export const resetTargetTempSuccess = action$ => action$
	.pipe(ofType(UPDATE_UI_TARGET_TEMP_SUCCESS), delay(50),
		map(() => ({ type: UPDATE_UI_TARGET_TEMP_TOGGLE_RESET, payload: { successToggle: false } })));

export const nestInfoEpic = combineEpics(
	changeTargetTemp, updateNestTargetTemp, resetTargetTempSuccess,
);
